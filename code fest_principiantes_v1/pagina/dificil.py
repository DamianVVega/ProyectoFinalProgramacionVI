...
def dificil_page(page: ft.Page):
    page.clean()
    page.title = "Difícil - Code Fest 2025"
    page.bgcolor = "#FFCF3D"
    ...
    ejercicios = EJERCICIOS["dificil"]
    contenedores = []

    for ej in ejercicios:
        estado_actual = page.session.get(f"dificil_{ej['id']}_estado")
        ...
        if estado_actual not in ["resuelto", "expirado"]:
            columna_items.append(
                ft.ElevatedButton(
                    ...
                    width=180,
                    disabled=bloqueado,
                    on_click=None if bloqueado else lambda e, ex=ej: resolver_page(page, "dificil", ex["id"])
                )
            )
